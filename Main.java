...
class Library {
				...
				public void addBook() {
								...
								for (String i : bookStore) {
												if (i == null) {   // if 'i' is null we go in, null means that block is empty
																bookStore[c] = x;  // we assign that block as x, but we dont havev access to 'i' in enhanced loop, so
				...
				public void showAvailableBooks() {
								...
								if (c == 0) {  // now if the counter c is zero, that means there were all the spaces null so we never made
